 ...
 * <br/>This method is used for Non-SMP registration and only supported by ios client. When enableAutoSelectSingleCert is set to true, if only one client certificate exists in application keychain,
 * then the xmlHttpRequest will automatically select it for any client certificate challenges. This method has no effect on Android or Windows client platforms. 
 * <br/>For Android client, the user must manually select the certificate when it is challenged by server at the first time, after that, the application will remember the user's selection and
