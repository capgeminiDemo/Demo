 ...
 * 1. https://issues.apache.org/jira/browse/CB-4740
 */
...
- (void)browserExit
{
    // kapsel customization, only call close success callback when the viewcontrol is dismissed in the dismiss completion block
    /*if (self.callbackId != nil) {
        CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
    ...
    // self.inAppBrowserViewController.navigationDelegate = nil;  //kapsel customization to set it to null in dismiss completion block
...
- (void)createViews
{
    ...
    if (_browserOptions.hidenavigation){
        [self.toolbar setItems:@[self.closeButton]];
    }
    else{
        [self.toolbar setItems:@[self.closeButton, flexibleSpaceButton, self.backButton, fixedSpaceButton, self.forwardButton]];  //original line
    }
...
{
    //kapsel customization
    [CDVUserAgentUtil setUserAgent:_prevUserAgent lockToken:_userAgentLockToken]; // Patch - CB-4740
    //end kapsel customization
    ...
    if ((self.navigationDelegate != nil) && [self.navigationDelegate respondsToSelector:@selector(browserExit)]) {
 	...
 	//kapsel customization. Sometime the webview does not get released immedately after closing the viewController
    //to avoid it messed up with the new inappbrowser instance, reset the webcontent and javcript data before
    //dismiss the webview 
        ...
        if ([self respondsToSelector:@selector(presentingViewController)]) {
            [[self presentingViewController] dismissViewControllerAnimated:NO completion:^{
                  if (self.navigationDelegate.callbackId != nil) {
                    ...
                    self.navigationDelegate.callbackId = nil;
                  }
            }];  //Kapsel customization: Change Animated to NO to avoid UI dead lock
        } else {
            ...
            if (self.navigationDelegate.callbackId != nil) {
                  CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                                                              messageAsDictionary:@{@"type":@"exit"}];
                [self.navigationDelegate.commandDelegate sendPluginResult:pluginResult callbackId:self.navigationDelegate.callbackId];
                self.navigationDelegate.callbackId = nil;  //Kapsel customization: Change Animated to NO to avoid UI dead lock
            }
        }
    });
}
