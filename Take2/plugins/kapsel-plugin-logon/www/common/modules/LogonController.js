                ...
                sap.logon.Core.isRegistered(function(bRegistered) {
                        if (!bRegistered) {
                            sap.logon.Core.useAfaria(
                                function() {
                                    sap.logon.Core.setUserCreationPolicy(logon, onCallRegistrationError, "certificate", context.appConfig.appID, context.smpConfig, context.appConfig.certificate);
                                },
                        ...
                        if (context.appConfig.certificate) {
                            if (context.appConfig.isForSMP) {
                                sap.logon.Core.isRegistered(function(bRegistered) {
                                        ...
                                        if (!bRegistered) {
                                            sap.logon.Core.setUserCreationPolicy(
                                                function() {
                                                    sap.logon.Core.provisionCertificate(logon, onCallRegistrationError, context.appConfig.certificate, !bRegistered, context.appConfig);
                                                },
                                                onCallRegistrationError, "certificate", context.appConfig.appID, context.smpConfig, context.appConfig.certificate);
                                        } else {
                                ...
                                sap.logon.Core.hasSecureStore(function(bHasStore) {
                                        sap.logon.Core.provisionCertificate(logon, onCallRegistrationError, context.appConfig.certificate, !bHasStore, context.appConfig);
                                    },
                                    onCallRegistrationError, context.appConfig.appID);
                            }
                        } else {
    ...
    var init = function(successCallback, errorCallback, applicationId, context, customView, credentialProviderID) {
        ...
        //document.addEventListener("onSapResumeSuccess", refreshAccessToken, false);
        document.addEventListener("onSapLogonSuccess", refreshAccessToken, false);
        }
    ...
    var saveTokens = function (accessToken, refreshToken) {
        sap.Logon.core.setSecureStoreObject(function () {
            utils.logJSON('access token has been stored successfully');
        ...
        sap.Logon.core.setSecureStoreObject(function () {
            utils.logJSON('refresh token has been stored successfully');
    ...
    var saveTokenEndpoint = function (tEndpoint) {
      ...
      sap.Logon.core.setSecureStoreObject(function () {
             utils.logJSON('token endpoint has been stored successfully');
    ...
    var FlowRunner = function(callbacks, pLogonView, pLogonCore, flowClass, flowContext) {
        ...
        onFlowSuccess = function onFlowSuccess() {
            ...
            if (sap.logon.IabUi.accessToken != null && sap.logon.IabUi.refreshToken != null) {

                saveTokens(sap.logon.IabUi.accessToken, sap.logon.IabUi.refreshToken);
                sap.logon.IabUi.accessToken = null;
                sap.logon.IabUi.refreshToken = null;
                saveTokenEndpoint(sap.logon.IabUi.tokenEndpoint);
                    sap.logon.IabUi.tokenEndpoint = null;
            } else if (sap.logon.LogonJsView.accessToken != null && sap.logon.LogonJsView.refreshToken != null) {
        ...
        var onSAMLAuthEvent = function(event) {
            ...
            if (event.url.indexOf(getSAMLConfig(context)["config"]["saml2.web.post.finish.endpoint.redirectparam"]) != -1) {
                ...
                if (screenID != null && screenID != "SCR_SAML_AUTH") {
                    // make a copy of the object. Otherwise, the busy flag is persisted across screens and causes problems.
                    var copiedContext = shallowCopy(screenContext);
                    if (screenContext) {
                        copiedContext.busy = true;
                    }
                    showScreen(screenID)(copiedContext);
                }
                ...
                // If SAML screen was the 1st screen shown to the user, we cannot display an error if reg fails.
                // Instead, we will invoke the app's error callback. 
                var errorCallback = ((screenID == null || screenID == "SCR_SAML_AUTH") ? onRegErrorNoPreviousWindow : onRegError);
                ...
                // windows does not start from a file:// url. Will need to revisit in Windows 10 when the webview will load all plugins.
                    ...
                    if (SAMLState == "notInit") {
                        ...
                        if (_providedContext.refreshSAMLSessionOnResume == "skip"){
                            utils.log("skip smal authentication " + _providedContext.refreshSAMLSessionOnResume);
                        }
                        else{
                            var samlConfigPlus = getSAMLConfig(_providedContext);
                            samlConfigPlus.serverHost = _providedContext.serverHost;
    ...
    var performSAMLAuth = function(onsuccess, onerror, registrationContext, appID){
        ...
        var errorListener = function(event){
             ...
             utils.log("performSAMLAuth errorListener:" + JSON.stringify(event));
        };
        ...
        if (!samlConfig){
            if (onerror){
             ...
             var errorCallback = function(e){
                        ...
                        var ret = confirm("Failed to connect, would you like to retry the request?");
                        if (ret == true){
    ...
    var refreshSession = function (onsuccess, onerror) {
        ...
        var serverHost = null;
        var serverPort = null;
        var serverProtocol = null;
        var appCid = null;

        // getting the server address before the request
        ...
        sap.Logon.core.getSecureStoreObject(function (context) {
            serverHost = context.serverHost;
            serverPort = context.serverPort;
            serverProtocol = !context.https || context.https == "false" ? "http" : "https";

            sap.Logon.unlock(function (connectionInfo) {
                // getting the application connection Id from the connection info
                appCid = connectionInfo.applicationConnectionId;
                sendRequest();
            }, function () {
                utils.log("sap.Logon.unlock failed");
            });
        ...
        var sendRequest = function () {
            ...
            var pingUrl = serverProtocol + "://" + serverHost + ":" + serverPort + "/odata/applications/v2/" +
                sap.Logon.applicationId + "/Connections('" + appCid + "')";

            sap.AuthProxy.sendRequest("GET", pingUrl, { "Authorization": "Bearer " + aToken }, null,
                function (result) { // success
                    utils.log("Ping success: " + result.status);
                    onsuccess(result);
                },
                function (error) { // error
                    utils.log("Ping error: " + error);
                    onerror(error);
                });
        };
    }
                        ...
                        if (result.status == 200 && result.responseText) {
                            ...
                            try {
                                ...
                                refreshSession(function (success) {
                                    ...
                                    console.log("Failed to refresh the session: " + error);
                                });
